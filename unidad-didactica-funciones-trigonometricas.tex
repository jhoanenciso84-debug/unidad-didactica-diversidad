...
\vspace{1.5cm}
\begin{center}
\itshape Sesi\'on de dos horas dedicada a la exploraci\'on h\'aptica de la
funci\'on $\operatorname{sen}(x)$.
\end{center}
...
\textbf{funci\'on seno} es la puerta de entrada natural a este conjunto de
funciones: su forma ondulada, acotada y peri\'odica permite introducir con
claridad los conceptos de m\'aximo, m\'inimo, per\'iodo, amplitud y rango. Para
estudiantes con baja o nula visi\'on, los recursos visuales convencionales
resultan inaccesibles sin una mediaci\'on pedag\'ogica adecuada.

Esta sesi\'on propone un canal de acceso alternativo: la \textbf{exploraci\'on
h\'aptica mediante una l\'amina en relieve del seno}. El estudiante recorre la
curva con los dedos, asociando cada forma y posici\'on a un valor matem\'atico
preciso. El conocimiento construido es matem\'aticamente id\'entico al que
obtiene un estudiante vidente; \'unicamente el canal sensorial es diferente.

La sesi\'on se fundamenta en el \textbf{Dise\~no Universal para el Aprendizaje
...
% ---- FILA INICIAL 1 ----
\multirow{2}{*}{\rotatebox{90}{\textbf{INICIAL}}}
& 5 min
...
% ---- FILA DESARROLLO 1 ----
\multirow{4}{*}{\rotatebox{90}{\textbf{DESARROLLO}}}
& 10 min
...
% ---- FILA DESARROLLO 4 ----
& 25 min
& \textbf{Aplicaci\'on y profundizaci\'on.} Localizaci\'on de puntos clave y verificaci\'on de la periodicidad recorriendo un segundo ciclo.
...
& Exploraci\'on guiada + dictado de valores.
...
Antes de tocar la l\'amina, el docente establece el marco con lenguaje
concreto:

\begin{itemize}[itemsep=3pt]
  \item Una \textbf{funci\'on} es una relaci\'on entre una entrada (el \'angulo
        $x$) y una salida (el valor de la funci\'on).
  \item La funci\'on seno \textbf{oscila}: sube y baja de forma repetida e
        indefinida.
...
\begin{itemize}[itemsep=3pt]
  \item \textbf{Localizaci\'on de puntos clave:} el estudiante ubica sobre la
        l\'amina los puntos $x=0$, $\pi/2$, $\pi$, $3\pi/2$ y $2\pi$, y dicta en
        voz alta el valor correspondiente del seno.
  \item \textbf{Ceros, m\'aximo y m\'inimo:} distingue con el tacto los lugares
        donde la curva cruza el nivel medio (ceros), alcanza su punto m\'as alto
        (cresta) y su punto m\'as bajo (valle).
  \item \textbf{Verificaci\'on de la periodicidad:} recorre un \textbf{segundo
        ciclo} y comprueba que la forma palpada es id\'entica a la del primero,
        confirmando que el seno se repite cada $2\pi$.
\end{itemize}

\begin{quote}
\itshape ``Si vuelves a empezar el recorrido, tus dedos sienten exactamente la
misma colina y el mismo valle. Eso es la periodicidad: la curva se repite sin
cambiar su forma.''
\end{quote}

\textbf{Pregunta de investigaci\'on:} \textit{``?`Reconoce el estudiante de
forma aut\'onoma que la forma del seno se repite cada $2\pi$?''}
...
\begin{enumerate}[itemsep=4pt]
  ...
  \item Explica con tus palabras qu\'e significa que el seno sea una funci\'on
        \textbf{peri\'odica} de per\'iodo $2\pi$.
  \item Describe la diferencia entre el m\'aximo, el m\'inimo y los ceros de la
        funci\'on.
  \item \textbf{Profundizaci\'on:} investiga un fen\'omeno real (por ejemplo el
        sonido o las olas del mar) que se comporte como una onda senoidal.
\end{enumerate}
...
aplicaci\'on de la funci\'on seno en el an\'alisis de se\~nales sonoras
(nociones iniciales de ondas).
